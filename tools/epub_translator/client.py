            ...
            try:
                ...
                with self._request_func(request, timeout=self.timeout) as response:
                    if response.status != 200:
                        ...
                        raise DeepSeekError(f"HTTP {response.status}: {error_body}")
                    raw = response.read()
                payload = json.loads(raw.decode("utf-8"))
                return self._extract_message(payload)
            except (json.JSONDecodeError,):
                # Treat JSON errors the same as API errors so they can be retried.
                if attempt == self.max_retries:
                    raise DeepSeekError("Invalid JSON response from DeepSeek API.")
